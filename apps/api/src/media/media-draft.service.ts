...
function buildUserPrompt(claim: string, context: string | null, passages: string): string {
...
export class MediaDraftService {
  ...
  async generate(claim: string, context: string | null): Promise<MediaDraftResult> {
    const query = [claim, context].filter((part) => part?.trim()).join("\n\n");
