...
function isSpaRoute(path: string): boolean {
  ...
  const match = normalized.match(/^\/requests\/([^/]+)$/);
  return Boolean(match && MEDIA_REFERENCE_PATTERN.test(match[1].toUpperCase()));
}
...
async function proxy(req: IncomingMessage, res: ServerResponse, env: ServerEnv): Promise<void> {
  ...
  try {
    ...
    sendJson(res, 502, { message: "The media desk is unreachable. Try again shortly." });
  }
}
...
export function mediaServerPlugin(env: ServerEnv): Plugin {
  return {
    name: "media-portal-server",
